...
const CartScreen = ({ navigation, route, orderslist, addOrder, deleteOrder, ...props }) => {
  ...
  useEffect(() => {
    getDataUser()
    setSubtotal(route.params.value.price)
  }, [])
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.containerItem}>
          <View style={{ flexDirection: 'row', alignItems: 'center', width: '80%' }}>
            <Image source={{ uri: `${baseURL}/${route.params.value.image}` }} style={{ height: 45, width: 45, marginRight: 15 }} />
            <View style={{ maxWidth: '85%' }}>
              <Text numberOfLines={1} style={{ fontWeight: '500', color: "#000" }}>{route.params.value.name}</Text>
              <Text numberOfLines={1} style={{ lineHeight: 25, fontSize: 11 }}>{route.params.value.description.slice(0, 80)}...</Text>
              <Text numberOfLines={1} style={{ fontWeight: '500' }}>Rp. {route.params.value.price}</Text>
            </View>
          </View>
        </View>
